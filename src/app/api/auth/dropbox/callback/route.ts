/**
 * Dropbox OAuth Callback Endpoint (PROJ-12 Phase 5)
 *
...
import { DropboxFileSystemAdapter } from '@/lib/import/adapters/dropbox-adapter'
...
export async function GET(request: Request) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL(
        `/settings/import-sources?oauth_error=${encodeURIComponent(errorDescription || error)}`,
        request.url
      )
    )
  }
  ...
  try {
    ...
    if (!storedCsrfToken || storedCsrfToken !== csrfToken) {
      console.error('[Dropbox OAuth Callback] CSRF token mismatch')
      return NextResponse.redirect(
        new URL(
    ...
    if (sourceError || !userSource) {
      console.error('[Dropbox OAuth Callback] User not authorized for source:', sourceId)
      return NextResponse.redirect(
        new URL(
    ...
    const tokens = await DropboxFileSystemAdapter.exchangeCodeForTokens(code)
    ...
    return NextResponse.redirect(
      new URL(
        `/settings/import-sources?oauth_success=dropbox&source_id=${sourceId}`,
        request.url
      )
    )
  } catch (error) {
