/**
 * Dropbox OAuth Authorization Endpoint (PROJ-12 Phase 5)
 *
 * Initiates OAuth flow for Dropbox access.
...
export async function GET(request: Request) {
  ...
  if (!process.env.DROPBOX_CLIENT_ID || !process.env.DROPBOX_CLIENT_SECRET) {
    return NextResponse.json(
      { error: 'Dropbox OAuth ist nicht konfiguriert' },
      { status: 500 }
    )
  }
  ...
  try {
    ...
    const authUrl = await DropboxFileSystemAdapter.getAuthUrl(state)

    return NextResponse.json({ auth_url: authUrl })
  } catch (error) {
    console.error('[Dropbox OAuth] Error generating auth URL:', error)
    return NextResponse.json(
